...
export default function SalesPage() {
  ...
  const generate = async (type: string, quote?: any) => {
    ...
    try {
      ...
      localStorage.setItem("sales_" + type, full);
      localStorage.setItem("sales_" + type + "_date", new Date().toDateString());
    } catch {
      ...
      <div style={{ maxWidth:800, margin:"0 auto", padding:24 }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:24, flexWrap:"wrap" as const }}>
          {TABS.map(tab => (
            <button
              ...
              style={{ padding:"10px 16px", borderRadius:8, border:`1px solid ${activeTab === tab.id ? C.accent : C.border}`, background:activeTab === tab.id ? C.accentDim : "transparent", color:activeTab === tab.id ? C.accent : C.muted, cursor:"pointer", fontWeight:activeTab === tab.id ? 700 : 400, fontSize:".84rem" }}
            >
              {tab.label}
            </button>
          ))}
              ...
              {completedQuotes.length === 0 ? (
                ...
                <div style={{ display:"flex", flexDirection:"column" as const, gap:8 }}>
                  {completedQuotes.slice(0, 5).map(q => (
                    <button key={q.id} onClick={() => setSelectedQuote(q)} style={{ padding:"12px 14px", borderRadius:8, border:`1px solid ${selectedQuote?.id === q.id ? C.accent : C.border}`, background:selectedQuote?.id === q.id ? C.accentDim : C.surface, color:C.text, cursor:"pointer", textAlign:"left" as const }}>
        ...
        {activeTab === "academy" && (
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
            ...
            <button onClick={() => generate("academy")} disabled={loading} style={{ padding:"12px 20px", borderRadius:8, border:"none", background:C.accent, color:"#000", fontWeight:700, cursor:loading ? "not-allowed" : "pointer", fontSize:".88rem" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column" as const, gap:12 }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
              ...
              <textarea
                ...
                placeholder="Ask anything about sales, pricing, handling objections..."
                rows={3}
                style={{ width:"100%", padding:"12px", borderRadius:8, border:`1px solid ${C.border}`, background:C.surface, color:C.text, fontSize:".84rem", fontFamily:"inherit", resize:"vertical" as const, boxSizing:"border-box" as const, outline:"none", marginBottom:12 }}
              />
              <button onClick={() => generate("ask")} disabled={loading || !question.trim()} style={{ padding:"12px 20px", borderRadius:8, border:"none", background:question.trim() ? C.accent : "rgba(217,123,79,0.3)", color:question.trim() ? "#000" : "rgba(255,255,255,0.3)", fontWeight:700, cursor:question.trim() ? "pointer" : "not-allowed", fontSize:".88rem" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column" as const, gap:12 }}>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:20 }}>
              <textarea
                ...
                placeholder="Save a line, script, or idea..."
                rows={3}
                style={{ width:"100%", padding:"12px", borderRadius:8, border:`1px solid ${C.border}`, background:C.surface, color:C.text, fontSize:".84rem", fontFamily:"inherit", resize:"vertical" as const, boxSizing:"border-box" as const, outline:"none", marginBottom:12 }}
              />
              <button
                ...
                style={{ padding:"10px 20px", borderRadius:8, border:"none", background:newNote.trim() ? C.accent : "rgba(217,123,79,0.3)", color:newNote.trim() ? "#000" : "rgba(255,255,255,0.3)", fontWeight:700, cursor:"pointer", fontSize:".84rem" }}
              >
            ...
            {notes.map(note => (
              <div key={note.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8, padding:16 }}>
                <div style={{ fontSize:".84rem", color:C.text, lineHeight:1.6 }}>{note.content}</div>
                <div style={{ fontSize:".7rem", color:C.muted, marginTop:8 }}>{new Date(note.created_at).toLocaleDateString()}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Content display */}
        {(loading || content) && activeTab !== "notes" && (
            ...
            {content && (
              <>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
                  ...
                  <button onClick={() => navigator.clipboard.writeText(content)} style={{ padding:"6px 12px", borderRadius:6, border:`1px solid ${C.border}`, background:"transparent", color:C.muted, cursor:"pointer", fontSize:".75rem" }}>
                    📋 Copy
                  </button>
                </div>
